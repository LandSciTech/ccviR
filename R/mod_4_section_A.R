    ...
    fluidRow(
      column(
        ...
        p("The Climate Change Exposure Index (CCEI) map is created by calculating ",
          ...
          "and the projected future temperature and moisture. The CCEI is broken ",
          "into four categories of exposure and the proportion of the non-breeding ",
          "range overlapping each category is used to determine the migratory exposure index value"),
        uiOutput(ns("ui_ccei")),
...
mod_A_server <- function(id, spatial, species, parent_session) {
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$comb_exp_tbl <- gt::render_gt({
      req(spat_res())
      get_exposure_table(spat_res(), "combined", clim_readme(), clim_readme()$brks_cmd)
