filter_applicable_qs <- function(vuln_df, tax_grp, plant_taxa = c("Vascular Plant", "Nonvascular Plant")){
  ...
  vuln_df <- vuln_df  %>%
    mutate(score = case_when(.data$Code == "C4b" & tax_grp %in% c(plant_taxa) ~
                               NA_real_,
