...
crs_use <- sf::st_crs(clim_vars[["mat"]])
range_poly <- read_sf(saved$rng_poly_pth[1])

assess_poly <- read_sf(saved$assess_poly_pth[1]) %>% 
  ccviR:::prep_polys(crs = crs_use, var_name = "Assessment area", 
                     poly_clip = clim_vars$clim_poly,
                     clip_name = "Climate polygon", 
                     quiet = TRUE)

range_poly <- range_poly %>% 
  ccviR:::prep_polys(crs = crs_use, var_name = "Range", 
                     poly_clip = clim_vars$clim_poly,
                     clip_name = "Climate polygon", 
                     quiet = TRUE)

range_poly_assess <- ccviR:::clip_poly(range_poly, assess_poly, "Range", "Assessment area",
                                       quiet = TRUE)
...
ggplot() +
  ...
  geom_sf(data = context_North_Am %>% st_transform(sf::st_crs(range_poly)) %>% 
            st_crop(comb_ext), 
          fill = "grey90", colour = "grey80") +
...
invisible(capture.output(
  bivar_exp_plot <- plot_bivar_exp(clim_vars$mat, clim_vars$cmd, assess_poly, range_poly_assess)
))
