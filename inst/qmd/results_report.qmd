...
#saved <- params$saved
#spatial_details <- params$spatial_details
...
if(length(params$saved) == 1 && params$saved == "none") {
  ...
  saved <- as.data.frame(params$saved) %>%
...
range_poly <- read_sf(here(saved$range_poly_pth[1]))
assess_poly <- read_sf(here(saved$assess_poly_pth[1]))
clim_vars <- get_clim_vars(here(saved$clim_dir_pth[1]), scenario_names = saved$scenario_name)
clim_readme <- fs::path(here(saved$clim_dir_pth[1], "climate_data_readme.csv")) %>%
...
```{r}
dials <- map2(saved$CCVI_index, saved$scenario_name, ~ {
  ccviR:::plot_index_gauge(
    .x, type = "", 
    codes = index_labs$code[-nrow(index_labs)],
    nms = index_labs$label[-nrow(index_labs)], 
    cols = index_labs$col[-nrow(index_labs)],
    ttl = .y)
})
...
div(style = "break-inside: avoid; break-after: auto",
    div(class = "grid", 
        !!!map(dials, ~div(class = "g-col-6", .x, style = "margin: auto"))
    )
)
```
