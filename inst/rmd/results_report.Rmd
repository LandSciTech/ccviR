...
## Species range
```{r range_map,fig.height= 3, fig.width=7, fig.cap= "Map of the species range (black) and the assessment area (grey outline)."}
...
comb_ext <- st_union(sf::st_bbox(range_poly_clim) %>% sf::st_as_sfc(),
                            sf::st_bbox(scale_poly) %>% sf::st_as_sfc() %>%
                              st_transform(st_crs(range_poly_clim)))
