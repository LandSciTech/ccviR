...
if(unique(out_data$common_name) == ""){
  ttl_name <- paste0("*", unique(out_data$species_name), "*")
} else {
  ttl_name <- paste0(unique(out_data$common_name),
                     " (*", unique(out_data$species_name), "*)")
}
...
vuln_qs <- out_data %>% 
  ...
  tidyr::separate(Score, into = paste0("Value", 1:4), fill = "right", sep = ", ",
                  convert = TRUE) %>%
  mutate(score = 1) %>% 
  filter_applicable_qs(out_data$tax_grp, c("Vascular Plant", "Nonvascular Plant")) %>% 
  filter(!is.na(score)) %>% 
  rowwise() %>% 
  mutate(Score = c(.data$Value1, .data$Value2, .data$Value3, .data$Value4) %>% 
           na.omit() %>% range() %>%  
           unique() %>% as.character() %>% 
           {text_spec(valueNms[.], background = cols2[.])} %>% paste0(collapse = " - "))
