...
test_that("spatial runs with all data or optional data",{
  res <- analyze_spatial(rng_high, assess, clim_vars, nonbreed, ptn, hs,
...
test_that("Non matching crs are handled reasonably", {
  ...
  expect_is(res <- analyze_spatial(rng_high_lcctrans, assess, clim_vars[c(1:2, 6)])$spat_table,
                 "data.frame")

  # make sure results are the same after transformed
  res2 <- analyze_spatial(rng_high, assess, clim_vars[c(1:2, 6)])$spat_table
...
test_that("Multiple polygons are merged", {
  ...
  res1 <- analyze_spatial(rng_high, assess, clim_vars[c(1:2, 6)])$spat_table
  res2 <- analyze_spatial(rng_high2, assess, clim_vars[c(1:2, 6)])$spat_table
...
test_that("works with mulitple clim scenarios",{
  ...
  res <- analyze_spatial(rng_high, assess, clim_vars_multi,
  ...
  res2 <- analyze_spatial(rng_high, assess, clim_vars,
                     non_breed_poly = nonbreed, hs_rast = stack(hs1, hs2, hs),
                     hs_rcl = matrix(c(0:7, c(0,1,2,2,2,2,2,3)), ncol = 2),
                     scenario_names = c("RCP2.6", "RCP4.5", "RCP8.5"))
  ...
  expect_error(analyze_spatial(rng_high, assess, clim_vars_multi,
                           non_breed_poly = nonbreed, hs_rast = hs,
                           hs_rcl = matrix(c(0:7, c(0,1,2,2,2,2,2,3)), ncol = 2),
                           scenario_names = c("RCP4.5", "RCP8.5")),
               "rasters must have one layer or")

  res3 <- analyze_spatial(rng_high, assess, clim_vars_multi,
                     non_breed_poly = nonbreed, hs_rast = hs,
                     hs_rcl = matrix(c(0:7, c(0,1,2,2,2,2,2,3)), ncol = 2),
                     scenario_names = c("RCP2.6", "RCP4.5", "RCP8.5"))
