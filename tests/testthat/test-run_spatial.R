...
test_that("Nonoverlaping poly and raster",{
  ...
  nonbreed <- sf::st_as_sf(data.frame(geometry = sf::st_geometry(d$range) - 1000000)) %>%
    sf::st_set_crs(sf::st_crs(d$range))
  ...
  expect_warning(analyze_spatial(d$range, d$assess, clim_vars[c(1:2, 4, 6)],
...
test_that("Non matching crs are handled reasonably", {
  ...
  expect_warning({
    rng_high_lccset <- sf::st_set_crs(d$range, value = "+proj=lcc +lon_0=-90 +lat_1=33 +lat_2=45")
  })
  ...
  rng_high_ncrs <- sf::st_set_crs(d$range, NA)
  ...
  # be error
  hs_terra2 <- terra::`crs<-`(d$hs_terra, "+proj=lcc +lon_0=-90 +lat_1=33 +lat_2=45")

  expect_error(
    expect_message(
      analyze_spatial(d$range, d$assess, d$clim_vars, NULL, d$ptn, hs_terra2,
                      hs_rcl = d$rng_chg_mat,
                      scenario_names = d$scn_nms),
  ...
  expect_error(
    analyze_spatial(d$range, d$assess, d$clim_vars, NULL, d$ptn, hs_terra3,
                    hs_rcl = d$rng_chg_mat,
                    scenario_names = d$scn_nms),
    "does not have a CRS") %>%
    suppressMessages()
...
test_that("Multiple polygons are merged", {
  rng_high2 <- sf::st_buffer(d$range, -10000) %>% bind_rows(d$range)
...
test_that("works with mulitple clim scenarios",{
  ...
  expect_message(
    res2 <- analyze_spatial(d$range, d$assess,
  ...
  expect_error(analyze_spatial(d$range, d$assess, d$clim_vars,
...
test_that("gives error for points early", {
  rng_high_pts <- d$range %>% sf::st_make_grid(what = "centers")
