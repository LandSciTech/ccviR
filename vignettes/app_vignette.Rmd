...
* Projected range change raster: The modeled change in the species range caused by climate change. The values of the raster cells should indicate whether the area was lost, maintained, gained or was never included in the range.  
...
The last graph shows the scores for each vulnerability factor in each scenario. The scores reflect the the answer to the quesiton and the exposure multiplier that was applied. 

![](./vulnq_plot.PNG){width=100%}

Once you are finished with reviewing the results you can save the data as a csv file.
...
This saving process should be used for convenience only. It is not robust to changes in the app overtime. Therefore, it is recommended that you also download the results as a csv and use this file for long term storage.
