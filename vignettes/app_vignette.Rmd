...
On the Exposure Results page there are interactive maps of climate change exposure. The real climate data will cover all of North America but for the demonstration we are using a subset from New Brunswick. The exposure maps are classified into 6 classes and the proportion of the species range in each class is displayed in the table. The exposure multiplier is determined by the level of exposure and is used to modify the sensitivity and adaptive capacity components of the index based on exposure to climate change. To view the map for different scenarios click the layer symbol and toggle each layer on or off. 
