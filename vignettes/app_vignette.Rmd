...
For the demonstration species all the required data is included in the package, but for a real species you will need to either, [download](https://drive.google.com/drive/folders/18mO5GDUmwi-nswhIAC36bmtYsvmqNQkH?usp=share_link) a prepared set of climate data, unzip it and store it in a convenient folder, or create your own (see `vignette("data_prep_vignette", package = "ccviR")`). 
...
## Saving the app
The state of the app can be saved by clicking the "Save progress" button at the bottom app at any point in the assessment. Choose a name and location to save a csv file with all the app content that is easy to remember, you will need to find it again to load the saved app. To load the app from a saved csv file click "Select file" under "Load data from a previous assessment" at the bottom of the Welcome page and select the saved csv file. 
