...
* Protected areas polygons: Optionally, polygons indicating protected areas in the assessment region.
...
Next, select the range polygon for the species ("rng_poly.shp"), the assessment area polygon ("assess_poly.shp"), and the physiological thermal niche polygon ("ptn_poly.shp"). Leave the non-breeding range blank since we are assuming this species is not migratory. 
...
Vulnerability questions are detailed questions that the index uses to score the species' indirect exposure, sensitivity, adaptive capacity and modeled or documented responses to climate change. Descriptions of how to interpret and score the questions are available in the NatureServe guidelines which are included as a pop-up if you click the "i" (Show guidelines) button. 
...
Some spatial components are optional and if not provided can be answered manually.
...
On the Index Results page, the first section highlights Data Completeness:
how many questions were answered in each section, whether a sufficient number have been answered,
and how many are missing evidence.
The index is more reliable when more questions are answered. 
Feel free to return to specific Sections to add more evidence or answer more questions if there is
insufficient Data Completeness.

![](./data_completeness.png){width=70% fig-align="center"}


Next, click "Calculate CCVI" to calculate the index value. 
Once finished, the first section presents the Climate Change Vulnerability Index and Migratory Exposure Index for each climate scenario. 
