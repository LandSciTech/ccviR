...
## Introduction
The ccviR package implements the (NatureServe Climate Change Vulnerability Index (CCVI))[https://www.natureserve.org/conservation-tools/climate-change-vulnerability-index] in an R package and Shiny App. The app allows all of the geospatial aspects of calculating the CCVI to be done in R, removing the need for separate GIS calculations. It also enables easier sensitivity analysis of the Index. 
...
To demonstrate how the data is prepared the maps below show the change in mean annual temperature which is then classified into 6 categories based on 1/2 the interquartile range. 
