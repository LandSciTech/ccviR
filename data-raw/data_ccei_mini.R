...
# Subsample out paths
out_dir <- path(path_package("extdata", package = "ccviR"), "ccei_test")
dir_create(out_dir)
dir_out_future <- dir_create(path(out_dir, "future"))
dir_out_hist <- dir_create(path(out_dir, "historical"))
...
# In South America
plot(ccei, type = "interval", breaks = c(0, 20, 100000))

(box1 <- sf::st_bbox(c(xmin = -75, xmax = -50, ymin = -15, ymax = 0), crs = 4326)) %>%
  ext() %>%
  crop(x = ccei, y = .) %>%
  plot(type = "interval", breaks = c(0, 1,3,5,10,15, 20, 100000))
...
# Add for comparison
crop(ccei_245, box) %>%
  writeRaster(path(out_dir, "ccei_ssp245.tif"), overwrite = TRUE)
